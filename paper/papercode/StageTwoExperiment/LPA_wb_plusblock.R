...
# Contains the LPAwb+ and the DIRTLPAwb+ algorithms modified to test the effect of turning stageII on or off.
# Author :  Stephen Beckett ( https://github.com/sjbeckett/weighted-modularity-LPAwbPLUS )
# MIT License

###############################################

LPA_wb_plusblockon <- function(MATRIX,initialmoduleguess=NA) {
	...
	outlist = StageTwo_LPAwbdashON(row_marginals,col_marginals,MATRIX,BMatrix,Matsum,redlabels,bluelabels,Qb_now)
	redlabels = outlist[[1]]
	bluelabels = outlist[[2]]
	Qb_now = outlist[[3]]
	BL = outlist[[4]]
	...
	return(list(Row_labels=redlabels, Col_labels=bluelabels, modularity=Qb_now, block= BL))
}

###############################################

LPA_wb_plusblockoff <- function(MATRIX,initialmoduleguess=NA) {
	...
	#Run Phase 2: Connect divisions from top-down if it improves Qb, then run
	#phase 1 again. Repeat till Qb cannot be improved.
	outlist = StageTwo_LPAwbdashOFF(row_marginals,col_marginals,MATRIX,BMatrix,Matsum,redlabels,bluelabels,Qb_now)
	redlabels = outlist[[1]]
	bluelabels = outlist[[2]]
	Qb_now = outlist[[3]]
	BL = outlist[[4]]
...
StageTwo_LPAwbdashON <- function(row_marginals,col_marginals,MATRIX,BMatrix,Matsum,redlabels,bluelabels, Qb_now) {
	block=0
		...
		if(NUMdiv > 1) {
			for(div1check in 1:(NUMdiv-1)) {
				...
				for(div2check in (div1check+1):NUMdiv) {
					...
					#Qb_now is current mod, QQ is mod with a merge of 2 comms
...
return(list(redlabels, bluelabels, Qb_now, block))
}


###############################################

StageTwo_LPAwbdashOFF <- function(row_marginals,col_marginals,MATRIX,BMatrix,Matsum,redlabels,bluelabels, Qb_now) {
	block=0
	divisionsFound = DIVISION(redlabels,bluelabels)
	NUMdiv = length(divisionsFound)
	IterateFlag = 1
		...
		if(NUMdiv > 1) {
			for(div1check in 1:(NUMdiv-1)) {
				...
				for(div2check in (div1check+1):NUMdiv) {
					...
					#Qb_now is current mod, QQ is mod with a merge of 2 comms
					QQ = WEIGHTEDMODULARITY2(BMatrix,Matsum,redlabels,bluelabels)#This is Qb_now!! - the statements within the following if loop are not called:
					if(QQ > Qb_now) { #If a division will improve modularity - find the best way to do this
						block=1
                    				FoundBetter = 0
                    				for(aa in 1:NUMdiv) {
